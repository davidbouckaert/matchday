...
function medicalTask(state: GameState): void {
  ...
  // het volledige plan alleen als er een voedingsdeskundige is om het te schrijven
  if (state.infrastructure.recoveryLevel >= 1) {
    ...
    const rijk = state.cash > weekLonen * 20 && staffSkill(state, 'voeding') > 0;
    state.medical.voeding = rijk ? 'volledig' : state.cash > weekLonen * 8 ? 'basis' : 'geen';
  }
}
