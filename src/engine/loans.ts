...
export function loanOffers(state: GameState): LoanOffer[] {
  ...
  const options = [
    { key: 'kort', label: 'Kaskrediet (1 jaar)', principal: 25_000, weeks: 52, extra: 0.01 },
    { key: 'middel', label: 'Investeringskrediet (3 jaar)', principal: 100_000, weeks: 156, extra: 0 },
    { key: 'lang', label: 'Infrastructuurlening (10 jaar)', principal: 400_000, weeks: 520, extra: 0.005 },
  ];
  return options
    .map((o) => {
      const principal = Math.min(o.principal, limit);
...
export function emergencyOffer(): LoanOffer {
