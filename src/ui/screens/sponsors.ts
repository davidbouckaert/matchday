        ...
        <td class="btns stack-sm">${
          d.kind === 'stadion'
            ...
            : `<button class="sm" data-action="sponsor-extra" data-id="${d.id}" ${askedNow ? 'disabled data-tip="Dit seizoen al gevraagd"' : 'data-tip="Hij denkt erover na: je hoort het antwoord volgende week in het weekrapport. De kans hangt af van zijn tevredenheid."'}>Extra bijdrage vragen</button>
               ...
               <button class="sm ghost danger" data-action="sponsor-cancel" data-id="${d.id}" data-confirm="Contract met ${esc(d.name)} stopzetten?" data-tip="Ingrijpend: het contract stopt meteen en ${euro(d.weekly)} per week valt weg. De plaats komt vrij, maar je andere sponsors kijken op (−2 tevredenheid) en je reputatie krijgt een tik.">Stopzetten</button>`
        }</td>
      </tr>`;
    })
    .join('');
