...
export function playBar(g: GameState, o: HeaderOpts & { open: number; urgent: boolean; tourLoop: boolean; tourReady: boolean }): string {
  ...
  return `<div class="playbar">
    <div class="pb-left">
      ...
      ${
        o.tourLoop
          ? `<button class="pb-tour" data-action="nav" data-id="overzicht" ${tipAttr('Terug naar je Bureau: daar staat de volgende stap van de rondleiding klaar.')}>
              📚 volgende stap → je Bureau
            </button>`
          : ''
      }
