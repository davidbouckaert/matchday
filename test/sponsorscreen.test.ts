...
describe('Het sponsorscherm', () => {
  ...
  it('zegt per plaats hoeveel er nog vrij is, als badge in de kop van elke tegel', () => {
    // de teller stond als voetnootje onder het invoerveld en werd daar door niemand
    // gezien — gemeld als "zie ik niet meer staan", terwijl hij er stond. En "9 van 16
    // bezet" is een rekensommetje; "nog 7 vrij" is het antwoord.
    const s = readyGame();
    const html = sponsorsScreen(s);
    const borden = s.sponsors.filter((d) => d.kind === 'bord').length;
    expect(html).to.contain(`nog ${KIND_MAX.bord - borden} vrij`);
    ...
    expect(html, 'wat op slot staat, zegt dat ook in de badge').to.contain('op slot');
  });
});
